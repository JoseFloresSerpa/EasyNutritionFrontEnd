<template>
  <v-card>
    ...
    <v-card-text>
      <v-data-table :headers="headers" :items="displayComplaints" :items-per-page="5" :search="search"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          ...
          <v-icon small class="mr-2" @click="navigateToEditComplaint(item.id)">mdi-pen</v-icon>
    ...
    <v-card-actions>
      <v-btn small color="primary" @click="navigateToAddComplaint">Add Complaint</v-btn>
      <v-btn small color="error" @click="removeAllComplaints">Remove All</v-btn>
    </v-card-actions>
  </v-card>
</template>
