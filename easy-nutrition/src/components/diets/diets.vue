<template>
  <v-card>
    ...
    <v-card-text>
      <v-data-table :headers="headers" :items="displayDiets" :items-per-page="5" :search="search"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          ...
          <v-icon small class="mr-2" @click="navigateToEditDiet(item.id)">mdi-pen</v-icon>
    ...
    <v-card-actions>
      <v-btn small color="primary" @click="navigateToAddDiet">Add Diet</v-btn>
      <v-btn small color="error" @click="removeAllDiets">Remove All</v-btn>
    </v-card-actions>
  </v-card>
</template>
